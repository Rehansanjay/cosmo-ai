...
/** The password is checked here — with a real mint — rather than on first
 *  use, so a wrong one is reported at the point it was typed instead of
 *  surfacing later as a failure to start a session. */
function UnlockStep({ onUnlock }: { onUnlock: (password: string) => void }) {
  ...
  const submit = async () => {
    ...
    setRejected(false);
    try {
      const res = await fetch('/token', { method: 'POST', headers: mintHeaders(password) });
  ...
  return (
    <div className="open-step">
      <div className="unlock">
        ...
        <span>Its Cosmo key stays server-side; the password lets this page mint its own short-lived tokens.</span>
            ...
            <aside className="panel">
              ...
              <details className="settings" open={HOSTED && !apiKey}>
                ...
                <p className="field-note">
                  {HOSTED ? (
                    <>
                      This deployment keeps its Cosmo key server-side; the password lets the page mint its
